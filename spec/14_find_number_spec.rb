...
describe FindNumber do
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:number_over) { double('random_number', value: 6) }

      subject(:game_over) { described_class.new(0, 9, number_over, 6) }
    ...
    context 'when guess and random_number are not equal' do
      let(:number_over) { double('random_number', value: 4) }

      subject(:game_over) { described_class.new(0, 9, number_over, 5) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      it 'updates min to the same value as max' do
        expect { game.update_range }.to change { game.min }.to eq(game.max)
      end
